...
} from "lib/reducers/user_campaigns"

export default function Page(props) {
  const { data: session } = useSession()
  const router = useRouter()
  const [showingNewCampaignModal, setShowingNewCampaignModal] = useState(false)
  const [managePlayersForCampaign, setManagePlayersForCampaign] =
    useState(undefined)
  const [userCampaigns, setUserCampaigns] = useReducer(
    UserCampaignsReducer,
  ...
  const addPlayer = async (campaign, invite) => {
    setUserCampaigns({
      type: CampaignActions.AddPlayer,
      value: { invite, campaign: campaign },
    })
  }

  const removePlayer = async (campaign, invite) => {
    setUserCampaigns({
      type: CampaignActions.RemovePlayer,
      value: { inviteId: invite.id, campaign: campaign },
    })
  }
    ...
    <Layout>
      ...
      <div className="pb-4 mb-5">
        ...
        <button
          ...
          onClick={() => {
            setShowingNewCampaignModal(true)
            setManagePlayersForCampaign(undefined)
          }}
        >
          {" "}
          New Campaign{" "}
          ...
          {userCampaigns.map(({ id, admin, campaign }) => {
            return (
              <tr key={id}>
                <td className="pr-10">
                  <button
                    className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-2 mr-4 rounded"
                ...
                <td className="p-2">
                  ...
                  {admin && (
                    <PlusCircleIcon
                      ...
                      onClick={() => setManagePlayersForCampaign(campaign.id)}
                    />
                  )}
                </td>
                <td className="p-2">
      ...
      {showingNewCampaignModal && (
        <NewCampaignModal
          hide={() => setShowingNewCampaignModal(false)}
          complete={createCampaign}
          defaultName={`New Campaign${
            userCampaigns.length == 0 ? "" : " " + (userCampaigns.length + 1)
          }`}
        />
      )}
      {
        <ManagePlayersModal
          campaign={
            !!managePlayersForCampaign &&
            userCampaigns
              .map((u) => u.campaign)
              .find((c) => c.id == managePlayersForCampaign)
          }
